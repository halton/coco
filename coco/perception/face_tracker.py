...
# ---------------------------------------------------------------------------
# dashboard-007-extend: runtime config hot-reload (face_detection_enabled 软开关)
# ---------------------------------------------------------------------------
# 模板复用自 coco/perception/face_id.py dashboard-007：每 30s 读
# ~/.cache/coco/runtime_config.json，找到 face_detection_enabled 字段就调
# tracker.set_enabled(...)，否则保持当前 enabled。不重启 thread / 不重 init
# camera / detector；纯软开关 + _tick early return。
_RUNTIME_CONFIG_PATH = os.path.expanduser("~/.cache/coco/runtime_config.json")
_FACE_TRACKER_RELOAD_INTERVAL_S = float(
    os.environ.get("COCO_CONFIG_RELOAD_INTERVAL_S", "30.0")
)
_last_face_tracker_check_ts: float = 0.0
...
def _maybe_reload_face_tracker_runtime(tracker: "FaceTracker") -> None:
    """周期 check runtime_config.json，按 face_detection_enabled 字段切 tracker.enabled.

    - 节流：上次 check < _FACE_TRACKER_RELOAD_INTERVAL_S 秒直接 return
    - 字段缺失 → 保持当前 enabled，不动
    - 字段存在 → 调 set_enabled（即使值相同也调一次，幂等）
    """
    global _last_face_tracker_check_ts
    now = time.monotonic()
    if (now - _last_face_tracker_check_ts) < _FACE_TRACKER_RELOAD_INTERVAL_S:
        return
    _last_face_tracker_check_ts = now
    cfg = _load_runtime_config()
    if "face_detection_enabled" not in cfg:
        return
    new_val = bool(cfg.get("face_detection_enabled", True))
    current = getattr(tracker, "enabled", True)
    if new_val == current:
        return
    try:
        tracker.set_enabled(new_val)
        log.info("face_tracker.hot_reload enabled %r -> %r", current, new_val)
...
class FaceTracker:
    ...
    def __init__(
        ...
        *,
        ...
        # 不停 thread / camera / detector，只是入口短路避免 detect/track 开销。
        self.enabled: bool = True

    def set_enabled(self, value: bool) -> None:
        """dashboard-007-extend: hot-reload 调；切 self.enabled，不影响 thread/camera。"""
    ...
    def _tick(self) -> None:
        ...
        try:
            _maybe_reload_face_tracker_runtime(self)
        except Exception:  # noqa: BLE001
            pass
        if not self.enabled:
            return
