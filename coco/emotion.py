...
- 衰减保持时长 60s（``COCO_EMOTION_DECAY_S``）：60s 后无新强情绪输入
...
- ``COCO_EMOTION_DECAY_S`` — 衰减保持时长（hold time / cutoff）秒数，
  超过该秒数后 effective 回 NEUTRAL（cliff cutoff，非 half-life）；
  默认 60，clamp [1, 3600]
...
class EmotionTracker:
    """情绪状态机：记录最新一次检测到的强情绪 + 保持时长（hold time / cutoff）衰减。

    - record(label, now): 记一次检测结果
    - effective(now): 返回当前生效的 emotion（衰减后回 NEUTRAL）
    - 保持时长 decay_s（cliff cutoff，非 half-life）：从 record() 起算
      超过 decay_s 秒后一刀切回 NEUTRAL
